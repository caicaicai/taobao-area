...
EOF;
     ...
     * 处理街道数据
     */
    private function processStreet()
    {
        foreach ($this->province_city as $val) {
            if(strlen($val['id']) == 6 && substr($val['id'], 4) != "00") {
                $response = file_get_contents("https://lsp.wuliu.taobao.com/locationservice/addr/output_address_town_array.do?l3=".$val['id']);
                $response = substr($response, 31, -4);
                if($response != "") {
                    $response = substr($response, 1, -1);
                    $response_arr = explode("],[", $response);
                    foreach($response_arr as $arr) {
                        $arr = substr($arr, 1, -1);
                        $arr_arr = explode("','", $arr);
                        $add                     = [];
                        $add['id']               = $arr_arr[0];//ID
